...
export default function NoidaHeroSection() {
  ...
  const [form, setForm] = useState({ name: "", phone: "", course_interested: "", branch: "Nexxtechs Noida" });
  ...
  const handleEnquirySubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 201 || response.status === 200) {
        setSubmitted(true);
        setForm({ name: "", phone: "", course_interested: "", branch: "Nexxtechs Noida" });
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 xl:gap-16 items-center">
          <div className="space-y-8">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-5xl xl:text-6xl tracking-tight font-black text-[#111827] dark:text-white leading-[1.1]" data-testid="hero-heading">
              Best IT Training Institute in{" "}<span className="gradient-text">Noida</span> –{" "}Nexxtechs
            </h1>
            <p className="text-base md:text-lg text-[#4B5563] dark:text-gray-400 leading-relaxed max-w-lg" data-testid="hero-subtext">
              Build the skills you need for a successful career in technology with Nexxtechs Noida, a practical and career-focused IT training institute in Sector 2, Noida. We provide industry-oriented training designed for students, freshers, and individuals looking to start or develop their careers in the IT industry.
            </p>
            <div className="flex flex-wrap gap-4">
              <Button onClick={() => window.dispatchEvent(new CustomEvent("openPopupEnquiry"))} className="group relative bg-[#84CC16] text-black font-semibold rounded-full px-8 py-6 text-sm md:text-base hover:bg-[#65A30D] transition-all duration-300 md:hover:-translate-y-1.5 active:translate-y-0 hover:shadow-[0_10px_30px_rgba(132,204,22,0.4)] active:shadow-none" data-testid="hero-enroll-btn">
                Contact Nexxtechs Noida <ArrowRight size={18} className="ml-2 transition-transform duration-300 group-hover:translate-x-1" />
              </Button>
